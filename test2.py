...
# 1.기계학습 모델 파일 로드
import joblib
model = joblib.load('________regression_model.pkl')
...
# 3.데이터 시각화
col1, col2, col3 = st.columns(_______)  # 몇 개의 컬럼으로 나눌까?
with col1:
...
b = st.number_input(' 공부시간 입력 ', value=_____ )     # 초기값은 0.0
...
if st.button('점수예측'):                          # 사용자가 '점수예측' 버튼을 누르면
        ...
        ____ = model.predict(input_data)         # model이 예측한 값을 p에 저장한다
